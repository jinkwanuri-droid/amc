...
export default function MonthView() {
  ...
  // 기본 공휴일 + DB에서 가져온 공휴일 합치기
  const baseHolidays = getHolidays(currentDate.getFullYear());
  const allHolidays = [
    ...baseHolidays,
    ...customHolidays.map(h => ({ date: format(new Date(h.date), 'yyyy-MM-dd'), name: h.name }))
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-white/50">
      <div className="grid grid-cols-7 border-b border-gray-200 bg-white/60 backdrop-blur-sm">
        {weekDays.map((day, i) => (
          <div key={day} className={`text-center py-3 text-sm font-bold ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-gray-500'}`}>{day}</div>
        ))}
      </div>
      ...
      <div className="flex-1 grid grid-cols-7 auto-rows-fr">
        {calendarDays.map((day) => {
          ...
          return (
            <div key={day.toString()} className={`border-r border-b border-gray-200/50 p-2 flex flex-col ${isDayToday ? 'bg-yellow-50/80' : 'bg-white/40'} ${!isCurrentMonth ? 'opacity-40' : ''}`}>
