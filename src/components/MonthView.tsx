  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-7 border-b border-gray-100 pb-4">
        {weekDays.map((day, i) => (
          <div key={day} className={`text-center text-sm font-semibold ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-gray-500'}`}>
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            <div key={day.toString()} className="min-h-[120px] border-r border-b border-gray-100/50 p-2">
              <div className={`
                ...
                ${isCurrentMonth && !isDayToday && day.getDay() === 0 ? 'text-red-500' : ''}
                ${isCurrentMonth && !isDayToday && day.getDay() === 6 ? 'text-blue-500' : ''}
              `}>
                {format(day, 'd')}
              </div>
              {/* 예약 데이터가 들어갈 공간 */}
