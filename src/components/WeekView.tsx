          ...
          {weekDays.map((day) => {
            ...
            return (
              <div key={day.toString()} className={`flex flex-col items-center justify-center py-3 border-r border-gray-200/50 ${isDayToday ? 'bg-yellow-50/80' : ''}`}>
                ...
                <span className={`text-2xl font-bold w-10 h-10 flex items-center justify-center rounded-full mt-1 ${isDayToday ? 'bg-blue-600 text-white shadow-md' : (holiday ? 'text-red-500' : 'text-gray-700')}`}>
                  {format(day, 'd')}
                </span>
              </div>
            );
          ...
          <div className="w-20 border-r border-gray-200/50 flex flex-col bg-white/40">
            {times.map((hour) => (
              <div key={hour} className="h-[60px] relative">
                <span className="absolute -top-2.5 right-3 text-xs font-bold text-gray-400">{hour.toString().padStart(2, '0')}:00</span>
              </div>
            ))}
          </div>
            ...
            {weekDays.map((day, i) => {
              ...
              return (
                <div key={i} className={`border-r border-gray-200/50 relative ${isToday(day) ? 'bg-yellow-50/80' : 'bg-white/20'}`}>
                  ...
                  {/* ⭐ 예약 블록 그리기 (Absolute Position) */}
                  {dayReservations.map((res) => {
                    const start = new Date(res.startTime);
                    const end = new Date(res.endTime);
                    
                    // Top 여백(px) = (시작시간 - 오전7시) * 60분 + 시작 분
                    const topPos = (start.getHours() - 7) * 60 + start.getMinutes();
                    // 높이(px) = 시작~종료 사이의 분(Minute)
                    const heightPos = differenceInMinutes(end, start);

                    return (
                      <div 
                        key={res.id} 
                        className="absolute left-1 right-1 bg-blue-100 border-l-4 border-l-blue-500 border border-blue-200/80 rounded shadow-sm p-1.5 overflow-hidden z-10 hover:bg-blue-200 transition cursor-pointer"
