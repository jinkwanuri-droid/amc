...
export const useCalendarStore = create<CalendarState>((set) => ({
  ...
  reservations: [],
  fetchReservations: async () => { const res = await fetch('/api/reservations'); const data = await res.json(); set({ reservations: Array.isArray(data) ? data : [] }); },
  rooms: [],
  fetchRooms: async () => { const res = await fetch('/api/rooms'); const data = await res.json(); set({ rooms: Array.isArray(data) ? data : [] }); },
  customHolidays: [],
  fetchCustomHolidays: async () => { const res = await fetch('/api/custom-holidays'); const data = await res.json(); set({ customHolidays: Array.isArray(data) ? data : [] }); }
}));
